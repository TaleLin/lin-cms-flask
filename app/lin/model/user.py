...
class User(InfoCrud):
    ...
    def select_page_by_group_id(cls, group_id, root_group_id) -> list:
        ...
        query = db.session.query(manager.user_group_model.user_id).filter(
            manager.user_group_model.group_id == group_id, manager.user_group_model.group_id != root_group_id)
    ...
    def count_by_id_and_group_name(user_id, group_name) -> int:
        stmt = db.session.query(manager.group_model.id.label(
            'group_id')).filter_by(soft=True, name=group_name).subquery()
        result = db.session.query(func.count(manager.user_group_model.id)).filter(
            manager.user_group_model.user_id == user_id, manager.user_group_model.group_id == stmt.c.group_id)
        count = result.scalar()
        return count
